<script lang="ts">
  ...
  function isScrapedMessageArray(value: unknown): value is ScrapedMessage[] {
    return (
      Array.isArray(value) &&
      value.every(
        (item) =>
  ...
  /** Re-inject content-chat script using manifest-driven file discovery. */
  async function reinjectContentChat(tid: number): Promise<void> {
    // Prefer manifest-registered content scripts for the target tab.
    // Falls back to discovering content-chat.js from registered scripts.
    const registered = await browser.scripting.getRegisteredContentScripts();
    const chatScript = registered.find(
      (s) =>
        s.js?.some((f) => f.includes('content-chat')) ?? false,
    );

    if (chatScript?.js?.length) {
      await browser.scripting.executeScript({
        target: { tabId: tid },
        files: chatScript.js,
      });
    } else {
      // Fallback: inject by known build output name (last resort).
      await browser.scripting.executeScript({
        target: { tabId: tid },
        files: ['content-scripts/content-chat.js'],
      });
    }
  }
  ...
  async function requestMessages(): Promise<void> {
    ...
    try {
      ...
      if (isConnectionError && tabId != null) {
        // Reload-first recovery: ask user to reload if tab context is stale.
        try {
          await reinjectContentChat(tabId);
  ...
  const selectedMessages = $derived(
    scrapedMessages == null ? [] : scrapedMessages.filter((_, i) => selectedIds.has(i)),
  );

  const hasSelection = $derived(selectedIds.size > 0);

  function buildBodyFromSelected(): string {
    return selectedMessages.map((m) => `${m.sender}: ${m.text}`).join('\n');
  }
  ...
  async function analyze(): Promise<void> {
    ...
    try {
      ...
      const response = await fetch(BACKEND_VERIFY_CONTENT_URL, {
        ...
        body: JSON.stringify({ title, body, comments: '', language }),
      });
      if (!response.ok) {
        verdict = '';
      ...
      verdictReasoning =
        typeof assessment?.VerdictReasoning === 'string' ? assessment.VerdictReasoning : '';
      const isReportable =
  ...
  async function submitReport(): Promise<void> {
    ...
    const reportDescriptionTrimmed = reportDescription
      .trim()
...
<section class="chat-panel panel">
  ...
  {:else if chatError}
    ...
    <div class="chat-error-actions">
      <button type="button" class="chat-action-button" disabled={chatLoading} onclick={requestMessages}>
        Try again
      </button>
  ...
  {:else if scrapedMessages != null}
        ...
        <div class="chat-report-prompt">
          ...
          <div class="chat-report-actions">
            <button
              type="button"
              class="chat-action-button"
              onclick={() => (showReportPrompt = false)}
            >No</button>
            <button
              type="button"
    ...
    {#if reportSuccess}
      <p class="body-copy" style="color: var(--success-green, #15803d);">
...
<style>
  ...
  .chat-action-button:hover:not(:disabled) { background: #e5e7eb; }
  ...
  .chat-action-button.primary:hover:not(:disabled) { background: #1c40dd; }
  .chat-action-button:disabled { opacity: 0.6; cursor: not-allowed; }

  .chat-error-actions { display: flex; flex-wrap: wrap; gap: 10px; margin-top: 8px; }
  ...
  .chat-list-actions { display: flex; gap: 8px; margin-bottom: 8px; }
  ...
  .chat-small-button:hover { background: #f3f4f6; }
  ...
  .chat-message-row:last-child { border-bottom: none; }
  ...
  .chat-text-snippet { flex: 1; min-width: 0; color: #6b7280; }
  ...
  .chat-report-prompt { margin-top: 12px; }

  .chat-report-actions { display: flex; gap: 8px; margin-top: 8px; }
  ...
  .chat-form-label { font-size: 12px; font-weight: 600; color: #374151; }

  .chat-input { padding: 8px 12px; font-size: 14px; border: 1px solid #d1d5db; border-radius: 6px; }

  .chat-textarea { resize: vertical; min-height: 60px; }
</style>